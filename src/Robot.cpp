...
void Robot::add_mission(Workbench *workbench_buy, Workbench *workbench_sell) {
    ...
    if (workbench_sell->now_material.size() +
            workbench_sell->material_is_reserved.count() + 1 ==
        workbench_sell->tot_material.size()) {
...
void Robot::perform_mission() {
    ...
    if (target_pos().x != -1) {
        // 到达买入处，他有商品，就尝试购买
        if (cal_distance(mission.des_buy->pos(), pos()) < 0.4 and
            mission.des_buy->have_product() and type() == 0) {
            buy();                // 生成指令到 instruct
        ...
        // 判断购买成功
        if (mission.flg_buy == 1 and type() > 0 and
            cal_distance(mission.des_buy->pos(), pos()) < 0.4) {
        ...
        // 到达卖出处并且他有空余原料格
        if (!mission.flg_sell and
            cal_distance(mission.des_sell->pos(), pos()) < 0.4 and
            mission.des_sell->find_material(mission.des_buy->type()) == 0 and
            type() > 0) {
            sell();                // 生成指令到 instruct
        ...
        // 判断售卖成功
        if (mission.flg_sell == 1 and type() == 0 and
            cal_distance(mission.des_sell->pos(), pos()) < 0.4) {
...
void Robot::change_target(Point des) { target_position = des; }
...
void Robot::buy() {
    Mission &mission = missions.front();
    double dis = cal_distance(position, mission.des_buy->pos()) +
                 cal_distance(mission.des_buy->pos(), mission.des_sell->pos());
...
void Robot::sell() { instruct.push_back({3}); }

void Robot::destroy() { instruct.push_back({4}); }

int Robot::ID() { return RobotID; }

int Robot::type() { return object_type; }

Point Robot::pos() { return position; }

Point Robot::target_pos() { return target_position; }

bool Robot::has_mission() { return missions.size() > 0; }

int Robot::workbench() { return workbench_type; }
...
Point Robot::corner_pos() {
    Point c[9] = {{0.5, 0.5}, {0.5, 49.5}, {49.5, 0.5}, {49.5, 49.5},
                  {25, 50},   {25, 0},     {0, 25},     {50, 25}};
    int t = 4;
...
double Robot::corner_dis() { return cal_distance(position, corner_pos()); }
