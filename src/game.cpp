...
void Game::avoid_wall23() {
    for (int i = 0; i < 4; i++) {
        if (robot[i].pos().x < 3 and robot[i].li_speed.x < -1 or
            robot[i].pos().x > 47 and robot[i].li_speed.x > 1 or
            robot[i].pos().y < 3 and robot[i].li_speed.y < -1 or
            robot[i].pos().y > 47 and robot[i].li_speed.y > 1) {
            if (robot[i].missions.size() and
                cal_distance(robot[i].pos(),
                             robot[i].missions.begin()->des_buy->pos()) < 1.2) {
        ...
        if (!robot[i].type()) {
            ...
            // }
            // if (robot[i].pos().x > 48.9 and fabs(robot[i].direction) < PI /
            // 2) {
            //     robot[i].instruct.push_back({0, 1});
            // }
...
void Game::avoid_wall4() {
    for (int i = 0; i < 4; i++) {
        ...
        if (robot[i].pos().x < 2.5 and robot[i].li_speed.x < -3 or
            robot[i].pos().x > 47.4 and robot[i].li_speed.x > 3 or
            robot[i].pos().y < 2.5 and robot[i].li_speed.y < -3 or
            robot[i].pos().y > 47.4 and robot[i].li_speed.y > 3) {
        ...
        if (robot[i].pos().x < 1.2 and robot[i].li_speed.x < -0.1 or
            robot[i].pos().x > 48.8 and robot[i].li_speed.x > 0.1 or
            robot[i].pos().y < 1.2 and robot[i].li_speed.y < -0.1 or
            robot[i].pos().y > 48.8 and robot[i].li_speed.y > 0.1) {
            robot[i].instruct.push_back({0, 0});
        }
    }
}
...
void Game::avoid_colision() {
    for (int i = 0; i < 4; i++) {
        for (int j = i + 1; j < 4; j++) {
            ...
            angle a =
                robot[i].direction - cal_dir(robot[i].pos(), robot[j].pos());
            // robot[j] 的速度与 ji 连线的夹角
            angle b =
                cal_dir(robot[j].pos(), robot[i].pos()) - robot[j].direction;
            ...
            if (dis < 5 and fabs(a) < PI / 6 and
                cal_distance(robot[j].pos(), robot[j].target_pos()) < 0.3) {
            ...
            if (dis < 5 and fabs(b) < PI / 6 and
                cal_distance(robot[i].pos(), robot[i].target_pos()) < 0.3) {
                if (robot[i].target_pos() == robot[j].target_pos()) {
            ...
            // 有迎头相撞的风险
            if ((dis > 5 and fabs(a) < PI / 4 and fabs(b) < PI / 4) or
                (dis < 5 and fabs(a) < PI * 0.5 and fabs(b) < PI * 0.5) or
                (dis < 5 and fabs(a) < PI / 6 and
                 cal_distance(robot[j].pos(), robot[j].target_pos()) < 0.3) or
                (dis < 5 and fabs(b) < PI / 6 and
                 cal_distance(robot[i].pos(), robot[i].target_pos()) < 0.3)) {
                double ti = PI, tj = PI;
                if (a < 0) {
                    ti *= -1;
                ...
                if (d1 < 1 and d2 > 3) {
                    ...
                    double d1 =
                        cal_distance(robot[i].pos(), robot[i].target_pos());
                    double d2 =
                        cal_distance(robot[j].pos(), robot[j].target_pos());
                    if (d1 < d2) {
                        robot[j].instruct.push_back({1, tj});
                    } else {
                        robot[i].instruct.push_back({1, ti});
...
void Game::calculate_frame() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (Task task : taskmanager.tasks) {
            if (wbtypes[7].size() == 0 and task.workbench_sell->type() != 9) {
                if (i == 0) {
                    if (!(task.workbench_buy->pos().y > 36 and
                          task.workbench_sell->pos().y > 36)) {
                        continue;
                    }
                ...
                if (i == 1) {
                    if (!(task.workbench_buy->pos().x > 40 and
                          task.workbench_buy->pos().x < 48 and
                          task.workbench_buy->pos().y < 33 and
                          task.workbench_buy->pos().y > 19 and
                          task.workbench_sell->pos().x > 40 and
                          task.workbench_sell->pos().x < 45 and
                          task.workbench_sell->pos().y < 33 and
                          task.workbench_sell->pos().y > 19)) {
                        continue;
                    }
            ...
            // 任务的原料或产品其一被预定了，直接跳过
            if (task.workbench_buy->check_reserved_product() or
                task.workbench_sell->check_reserved_material(
                    task.workbench_buy->type())) {
                continue;
            }
            ...
            for (int j = 0; j < 4; j++) {
                ...
                if (robot[i].missions.rbegin()->des_sell ==
                        robot[j].missions.rbegin()->des_sell and
                    robot[i].time_to_free() > robot[j].time_to_free()) {
                    flg = 1;
                    break;
                }
            }
    ...
    if (mapID() != 3) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = i + 1; j < 4; j++) {
                if (robot[i].corner_pos().x != -1 and
                    robot[i].corner_pos() == robot[j].corner_pos()) {
                    int t =
                        robot[i].corner_dis() < robot[j].corner_dis() ? j : i;
                    double dir_bias =
                        robot[t].direction -
                        cal_dir(robot[t].position, robot[t].corner_pos());
                    if (dir_bias > PI) {
                        dir_bias -= 2 * PI;
                    } else if (dir_bias < -PI) {
                        dir_bias += 2 * PI;
    ...
    for (int i = 1; i <= 100; i++) {
        ...
        for (int j = 1; j <= 100; j++) {
            if (gamemap[i][j] >= '1' and gamemap[i][j] <= '9') {
                ...
                workbench[++num_workbench].position = {0.25 + 0.5 * (j - 1),
                                                       49.75 - 0.5 * (i - 1)};
